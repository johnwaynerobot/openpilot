...
from selfdrive.car.kia_soul import kia_soul
#from selfdrive.car.honda.values import AH, CruiseButtons, CAR
from selfdrive.car.kia_soul.values import AH, CruiseButtons, CAR  #2018.09.02 DV add for Kia soul
...
class CarController(object):
  ...
  def update(self, sendcan, enabled, CS, frame, actuators, \
    ...
    #BRAKE_MAX = 1024/4 2018.09.02 move into class
    ...
    idx = frame % 4  #2018.09.02 this mod it get the remainder??
    can_sends.append(kia_soul.create_steering_control_enable(self.packer, apply_steer, lkas_active, CS.CP.carFingerprint, idx))
    can_sends.append(kia_soul.create_steering_control(self.packer, apply_steer, lkas_active, CS.CP.carFingerprint, idx))
    can_sends.append(kia_soul.create_steering_control_disable(self.packer, apply_steer, lkas_active, CS.CP.carFingerprint, idx))
    ...
    if CS.CP.radarOffCan:
      ...
      #2018.09.02 Paras already change here for oscc kia soul kit
      if (frame % 2) == 0:
        idx = (frame / 2) % 4
        can_sends.append(
          kia_soul.create_brake_enable_soul(self.packer, apply_brake, pcm_override,
                                      pcm_cancel_cmd, hud.chime, hud.fcw, idx)) #enable brake command
        can_sends.append(
          kia_soul.create_brake_command_soul(self.packer, apply_brake, pcm_override,
                                      pcm_cancel_cmd, hud.chime, hud.fcw, idx)) #creating brake command
        can_sends.append(
          kia_soul.create_brake_disable_soul(self.packer, apply_brake, pcm_override,
                                      pcm_cancel_cmd, hud.chime, hud.fcw, idx)) #disable brake command
        can_sends.append(
          kia_soul.create_brake_command(self.packer, apply_brake, pcm_override,
                                      pcm_cancel_cmd, hud.chime, hud.fcw, idx)) #creating brake command for chime & FCW
        ...
        if CS.CP.enableGasInterceptor:
          ...
          can_sends.append(kia_soul.create_gas_command_enable(self.packer, apply_gas, idx))
          can_sends.append(kia_soul.create_gas_command(self.packer, apply_gas, idx))
          can_sends.append(kia_soul.create_gas_command_disable(self.packer, apply_gas, idx))
